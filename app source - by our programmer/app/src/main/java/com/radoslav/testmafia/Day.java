...
public class Day extends AppCompatActivity {
    ...
    ArrayList<String> toBeKilled = new ArrayList<>();
...
//                timer.setText("Remaining: " + millisUntilFinished / 1000);
            ...
            public void onFinish() {
                ...
                Firebase ref = new Firebase(Constants.firebaseUrl).child(UserDetails.group +"/Day"+(UserDetails.night-1));
                ref.addValueEventListener(new ValueEventListener() {
                                              @Override
                                              public void onDataChange(DataSnapshot snapshot) {
                                                  for (DataSnapshot child : snapshot.getChildren()) {
                                                      toBeKilled.add(child.getKey());
                                                      if(toBeKilled.contains(child.getKey())){
